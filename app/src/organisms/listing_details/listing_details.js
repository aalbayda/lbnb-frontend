...
const ListingDetails = (props) => {
  ...
  const rooms_dummy = [
   ...
   // to fetch rooms
    const[rooms, setRooms]=React.useState(rooms_dummy);
    useEffect(()=>{
    ...
    const min_max_capacity= String(props.min_capacity) + " - "+ String(props.max_capacity);
    const [max_price, setPrice]=React.useState(props.max_price)
    const [capacity, setCapacity]=React.useState(min_max_capacity);
    const[image, setRoomPIc]=React.useState(props.image);
    ...
    const handleClick=(room)=>{
      ...
      //to fetch room image 
        axios.post(url+"/room/get-room-pic", {
    ...
    const roomItems= rooms.map(
        (room) => {
            return(
                <button key={room.ROOM_ID} className="r-add-style" onClick={()=> handleClick(room)}> {room.ROOM_NAME}</button>
            )
        }
    )
    ...
    <Container>
      <Row className="listing-detials">
        ...
        <Col className="heart-icon-col">
          <HeartReact userName={userName} accomName={accommName} isFavorite={isFavorite}/>
        </Col>
      </Row>
    </Container>
  );
